...
class FullADataMySQLInserter:
    ...
    def insert_market_data_parallel(self, max_stocks=None):
        ...
        try:
            # 获取所有股票代码
            if max_stocks:
                self.cursor.execute(f"SELECT stock_code FROM stock_info ORDER BY stock_code LIMIT {max_stocks}")
            else:
                self.cursor.execute("SELECT stock_code FROM stock_info ORDER BY stock_code")
    ...
    def _process_single_stock_market_data(self, stock_code):
        ...
        try:
            ...
            if df.empty:
                logger.warning(f"⚠️ {stock_code} 无历史行情数据")
                return False
            ...
            try:
                ...
                for _, row in df.iterrows():
                    try:
                        ...
                        trade_time = str(row.get('trade_time')) if row.get('trade_time') else None
