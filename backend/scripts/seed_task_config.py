...
TASKS = [
    ...
    ("index_market_sync", 1, "30 18 * * 0-4", {}),                  # 每周一至周五 18:30（指数行情，不依赖个股日线）
    # ---------- ⚠️ 日线闸门：以下任务以「个股日线」为输入，必须排在 stock_daily_incr 完成之后 ----------
    # 实测 stock_daily_incr 常态耗时 15~50 分钟（19:00 起跑，最慢约 19:50 完成），
    # 故原 18:45 / 19:30 都太早 —— 2026-09-16 事故：快照写入半量 2820/5119 行，
    # 连锁导致 stock_info_sync 的名单护栏拒绝执行、连续 6 个班次失败，A 股名单停更。
    # 另注：index_market_sync(18:30) 虽早于日线，但其输出仅指数行情，不受影响。
    ("stock_daily_incr", 1, "0 19 * * 0-4",                         # 每周一至周五 19:00
     {"adjust": "qfq", "days_back": 15, "max_stocks": 0}),
    ("market_style_sync", 1, "5 20 * * 0-4", {}),                   # 每周一至周五 20:05（须晚于 daily_incr 完成）
    ("market_current_sync", 1, "15 20 * * 0-4", {}),                # 每周一至周五 20:15（须晚于 daily_incr 完成）
