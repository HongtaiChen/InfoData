...
# 配置日志
def setup_logging():
    log_file = f"daily_update_{datetime.now().strftime('%Y%m%d')}.log"
    logging.basicConfig(
        level=logging.INFO,
...
class DailyDataUpdater:
    def __init__(self, config_file='config_full.ini'):
        """初始化每日数据更新器"""
    ...
    def save_update_log(self):
        ...
        log_file = 'daily_update_history.json'
        history = []
    ...
    def update_current_market(self):
        ...
        try:
            ...
            logger.info(f"📊 准备更新 {len(stock_codes)} 只股票实时行情")
            
            # 清空当日实时行情
            today = datetime.now().strftime('%Y-%m-%d')
            self.cursor.execute("DELETE FROM stock_market_current WHERE DATE(update_time) = %s", (today,))
            
            success_count = 0
            batch_size = 100  # 实时行情API批量处理大小
            ...
            for i in range(0, len(stock_codes), batch_size):
                ...
                try:
                    logger.info(f"📈 处理第 {batch_num}/{total_batches} 批，股票数量: {len(batch_codes)}")
                    
                    # 获取实时行情
                    df = adata.stock.market.list_market_current(code_list=batch_codes)
                    
                    if not df.empty:
                        count = self._insert_current_market_batch(df)
                        success_count += count
                        logger.info(f"✅ 第 {batch_num} 批成功插入 {count} 只股票实时行情")
                    
                    # 请求延迟
                    time.sleep(self.request_delay)
                    
                except Exception as e:
                    error_msg = f"第 {batch_num} 批处理失败: {str(e)}"
                    logger.error(f"❌ {error_msg}")
                    self.update_stats['errors'].append(error_msg)
                    continue
            
            self.update_stats['current_market'] = success_count
            logger.info(f"✅ 实时行情更新完成: {success_count} 条记录")
            
        except Exception as e:
            error_msg = f"更新实时行情失败: {str(e)}"
            logger.error(f"❌ {error_msg}")
            self.update_stats['errors'].append(error_msg)
        ...
        for _, row in df.iterrows():
            try:
                ...
                batch_data.append((
                    ...
                    float(row.get('percent')) if row.get('percent') is not None else None,
                ...
                logger.warning(f"处理股票 {row.get('code', 'unknown')} 实时行情失败: {str(e)}")
                continue
